...
trace_module(Module, Options) ->
    Opts = genapp_opt:validate(Options, ?OPTIONS_SCHEMA),
    start_tracer(Opts),
    dbg_tpl(Module, Opts),
    dbg_calls().
...
message_flags(Opts, Acc) ->
    case genapp_opt:value(send_only, Opts) of
        true -> [s|Acc];
        false ->
            case genapp_opt:value(receive_only, Opts) of
                true -> [r|Acc];
                false -> [m|Acc]
            end
    end.
...
process_event_flags(Opts, Acc) ->
    case genapp_opt:value(process_events, Opts) of
        true -> [p|Acc];
        false -> Acc
    end.
...
tracer(Opts) ->
    Pattern = pattern_match_spec(genapp_opt:value(pattern, Opts)),
    Out = trace_device(genapp_opt:value(file, Opts)),
...
match_spec(Opts) ->
    case genapp_opt:value(no_return, Opts) of
	true ->
	    [{'_', [], []}];
	false ->
	    [{'_', [], [{return_trace}]}]
